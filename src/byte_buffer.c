...
{
	char *data_p = (char *) calloc (initial_size, sizeof (char));

	if (data_p)
		{
			ByteBuffer *buffer_p = (ByteBuffer *) malloc (sizeof (ByteBuffer));
...
bool ResizeByteBuffer (ByteBuffer *buffer_p, size_t new_size)
{
	...
	char *new_data_p = (char *) calloc (new_size, sizeof (char));
